...
if ( ! class_exists( 'M2_Theme_Wizard_Install_API' ) ) {

	/**
	 * Define M2_Theme_Wizard_Install_API class
	 */
	class M2_Theme_Wizard_Install_API {
		...
		public function do_theme_install() {
			...
			$data    = array();
			$success = true;
			$message = esc_html__( 'The theme is succesfully installed. Activating...', 'm2-theme-wizard' );
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( ! isset( $skin->result->errors['folder_exists'] ) ) {
					...
					$message = esc_html__( 'The theme has been already installed. Activating...', 'm2-theme-wizard' );
				}
				...
				$message = esc_html__( 'Unable to connect to the filesystem. Please confirm your credentials.', 'm2-theme-wizard' );
		...
		public function do_plugin_install( $slug = null ) {
			...
			$data    = array();
			$success = true;
			$message = esc_html__( 'Plugins wizard installed and activated. Redirecting...', 'm2-theme-wizard' );
		...
		public function adjust_plugin_dir( $source, $remote_source, $upgrader ) {
			...
			if ( empty( $_REQUEST['action'] ) && 'm2_theme_wizard_install_plugins_wizard' !== $_REQUEST['action'] ) {
				return $source;
			}
		...
		public function adjust_theme_dir( $source, $remote_source, $upgrader ) {
			...
			// Ensure that is Wizard installation request
			if ( empty( $_REQUEST['action'] ) && 'm2_theme_wizard_install_parent' !== $_REQUEST['action'] ) {
				return $source;
			}
			...
			if ( true === $wp_filesystem->move( $from_path, $to_path ) ) {
				 ...
				 */
				do_action( 'm2-theme-wizard/source-rename-done', $theme_data );
				...
				return new WP_Error(
					'rename_failed',
					esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'm2-theme-wizard' ),
...
/**
 * Returns instance of M2_Theme_Wizard_Install_API
 *
 * @return object
 */
function m2_theme_install_api( $url = null ) {
	return M2_Theme_Wizard_Install_API::get_instance( $url );
}
