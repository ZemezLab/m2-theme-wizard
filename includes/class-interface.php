...
if ( ! class_exists( 'M2_Theme_Wizard_Interface' ) ) {

	/**
	 * Define M2_Theme_Wizard_Interface class
	 */
	class M2_Theme_Wizard_Interface {
		...
		public function register_page() {
			add_management_page(
				esc_html__( 'M2 Theme Wizard', 'm2-theme-wizard' ),
				esc_html__( 'M2 Theme Wizard', 'm2-theme-wizard' ),
				'manage_options',
				m2_theme_wizard()->slug(),
		...
		public function get_page_link( $step = 'verification' ) {
			...
			return add_query_arg(
				array(
					'page' => esc_attr( m2_theme_wizard()->slug() ),
					'step' => esc_attr( $step ),
				),
				$base
			);
		...
		public function get_template( $template, $data = false ) {
			...
			$file = locate_template( m2_theme_wizard()->slug() . '/' . $template );

			if ( ! $file ) {
				$file = m2_theme_wizard()->path( 'templates/' . $template );
			}

			$file = apply_filters( 'm2-theme-wizard/template-path', $file, $template );
...
/**
 * Returns instance of M2_Theme_Wizard_Interface
 *
 * @return object
 */
function m2_theme_interface() {
	return M2_Theme_Wizard_Interface::get_instance();
}
